...
All player-facing UI: crosshair prompts, inventory, equipment, trade/loot, context menus, conversation, quest tracker, pause, settings, sleep, confirmation prompts, and save/load. Each feature is its own `MenuSystem`, and shared ownership prevents gameplay input from leaking into blocking menus.

The current item/shop/inventory surface affects UI in three places:
...
- The player inventory panel uses authored category tabs and search fields from the prefab; runtime code filters and binds them, but does not generate the menu shell.
...
- [ConfirmationPromptSystem.cs](../../Assets/Scripts/UserInterface/ConfirmationPromptSystem.cs): reusable confirmation prompt.
...
- [InventoryCategoryTabs.cs](../../Assets/Scripts/UserInterface/InventoryCategoryTabs.cs): authored inventory tab strip, selected category state, and item-category matching.
- [InventoryCategoryTabButton.cs](../../Assets/Scripts/UserInterface/InventoryCategoryTabButton.cs): one clickable inventory category tab with hover/active/disabled visuals.
...
- Inventory tabs call `InventoryUI.SetCategoryFilter(...)` through `InventoryCategoryTabs.Select(...)`.
...
The inventory panel is prefab-authored. `InventoryToggle` only opens/closes the assigned panel, claims UI input ownership, and hides tooltip/context-menu state on close. It no longer creates or configures a broader player-menu tab shell.

`InventoryUI` owns the visible slot list:

- Category filtering is driven by `InventoryCategoryTabId`: all items, weapons, armor, food/drink, potions, and miscellaneous. The category checks query gameplay tags first.
- Search uses an authored `TMP_InputField` named `InventorySearchInput` or `SearchInput`.
- Visible slots are sorted by display name.
- `InventorySlotUI` uses compact row stats by default. Table columns are shown only when the prefab wires authored `WeightText`, `DamageText`, `ArmorText`, and `ValueText` labels.

Category rules are centralized on `InventoryCategoryTabs`:

- Weapons include `Item.Weapon`, `Item.Tool`, `Item.Fishing.Rod`, and equipable items whose resolved equip domain is `WeaponTool`.
- Armor includes `Item.Armor` and equipable items whose resolved equip domain is `Armor`.
- Food/drink uses `Item.Food` and `Item.Drink`.
- Potions include `Item.Potion` plus usable consumables with use effects.
- Miscellaneous is the fallback after weapon, armor, food/drink, and potion checks.
...
- Inventory search, tabs, and table stat columns are authored UI references. Missing references should degrade to compact rows instead of creating runtime controls.

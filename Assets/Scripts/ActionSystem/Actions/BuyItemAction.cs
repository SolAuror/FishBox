...
{
    /// <summary>
    /// Discrete action: player buys an item from an NPC via gold exchange.
    /// Delegates to TradeController — no direct Inventory mutation here.
    /// Completes immediately.
    /// </summary>
    public class BuyItemAction : ItemAction
    {
        ...
        public BuyItemAction(InventorySlot slot, Inventory playerInventory, Inventory npcInventory)
        {
            _slot = slot;
        ...
        {
            Succeeded = TradeController.BuyItem(_slot, _playerInventory, _npcInventory);
            Complete();
        }
    }
}
